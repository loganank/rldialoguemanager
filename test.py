...
class DQN(nn.Module):
    ...
    def __init__(self, num_actions):
        ...
        self.layer1 = nn.Linear(1, 5404)
        self.layer2 = nn.Linear(5404, 5404)
        self.layer3 = nn.Linear(5404, num_actions)
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            emotions = torch.tensor(state['emotions'][0], dtype=torch.float32, device=device)
            sentences = state['sentences'][0].to(device)
            emotions_and_sentences = torch.cat((emotions, sentences), dim=0)
